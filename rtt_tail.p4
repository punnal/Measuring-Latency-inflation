...
#define RTT_THRESHOLD 2000000000
...
#define PKT_TYPE_INBOUND true
#define PKT_TYPE_OUTBOUND false
...
control SwitchIngress(
        ...
        table tb_decide_packet_type {
            ...
            const entries = {
                ...
                (TCP_FLAGS_A, _ ): mark_ACK();
                (TCP_FLAGS_A+TCP_FLAGS_P, _ ): mark_ACK();
                (_,_): mark_SEQ();
        ...
        #define TS_EXPIRE_THRESHOLD (5000*1000*1000)
        //500ms
        ...
        action exec_table_2_insert(){
            ig_md.table_1_read=table_2_insert.execute(ig_md.hashed_location_1);
        }
        ...
        RegisterAction<paired_32bit, _, bit<32>>(reg_table_2) table_2_tryRead= {  
            void apply(inout paired_32bit value, out bit<32> rv) {    
                ...
                #define current_entry_matched2 (in_value.lo==ig_md.pkt_signature)
                //#define timestamp_legitimate  ((TIMESTAMP-in_value.hi)<TS_LEGITIMATE_THRESHOLD)
                
                if(current_entry_matched2) //&& timestamp_legitimate)
                {
                    value.lo=0;
                    value.hi=0;
                    rv=in_value.hi;
        ...
        action exec_table_2_tryRead(){
            ig_md.table_1_read=table_2_tryRead.execute(ig_md.hashed_location_1);
        }
        // Table 3

        Register<paired_32bit,_>(32w65536) reg_table_3;
        //lo:signature, hi:timestamp
        
        RegisterAction<paired_32bit, _, bit<32>>(reg_table_3) table_3_insert= {  
            void apply(inout paired_32bit value, out bit<32> rv) {          
                rv = 0;                                                    
                paired_32bit in_value;                                          
                in_value = value;                 
        ...
        action exec_table_3_insert(){
            ig_md.table_1_read=table_3_insert.execute(ig_md.hashed_location_1);
        }
        
        RegisterAction<paired_32bit, _, bit<32>>(reg_table_3) table_3_tryRead= {  
            void apply(inout paired_32bit value, out bit<32> rv) {    
                rv=0;
                paired_32bit in_value;                                          
                in_value = value;     
                
                #define current_entry_matched3 (in_value.lo==ig_md.pkt_signature)
                //#define timestamp_legitimate  ((TIMESTAMP-in_value.hi)<TS_LEGITIMATE_THRESHOLD)
                
                if(current_entry_matched3) //&& timestamp_legitimate)
                {
                    value.lo=0;
                    value.hi=0;
                    rv=in_value.hi;
        ...
        action exec_table_3_tryRead(){
            ig_md.table_1_read=table_3_tryRead.execute(ig_md.hashed_location_1);
        }

        action set_rtt(){
            ig_md.rtt = TIMESTAMP-ig_md.table_1_read;
        }
        Register<bit<32>,bit<32>>(1) exceed;

        RegisterAction<bit<32>,bit<1>,bit<32>>(exceed) calc_exceed = {
            void apply(inout bit<32> register_data, out bit<32> value){
                register_data = 0;
                bool cond = ig_md.rtt > RTT_THRESHOLD;
                if(cond){
                    register_data = 1;
                }
                value = register_data;
        ...
        action set_exceed(){
            ig_md.exceed = calc_exceed.execute(0);
        }
        ...
        action hit(mac_addr_t dst_addr ,PortId_t port) {
        ...
        apply {
            ...
            */
            ...
            if(ig_md.table_1_read > 0 && ig_md.pkt_type==PKT_TYPE_ACK){
                set_rtt();
                set_exceed();
                if(ig_md.exceed > 0){
                    send_update_table.apply();
...
control SwitchIngressDeparser(
    ...
    apply {
        if (ig_intr_dprsr_md.mirror_type == 1){
            mirror.emit<update_digest_h>(ig_md.sid, {
                ig_md.table_1_read,
                ig_md.rtt,
                //ig_md.expected_ack,
                hdr.ipv4.src_addr, 
